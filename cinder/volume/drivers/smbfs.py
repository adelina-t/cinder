...
class SmbfsDriver(nfs.NfsDriver):
    ...
    def get_volume_format(self, volume, qemu_format=False):
        ...
        if qemu_format and volume_format == 'vhd':
            volume_format = 'vpc'
    ...
    def _create_windows_image(self, volume_path, volume_size, volume_format):
        """Creates a VPC or VHDX file of a given size."""
    ...
    def create_snapshot(self, snapshot):
        ...
        backing_file = os.path.join(self.local_volume_dir(snapshot['volume']),
                                    backing_filename)
        self._create_snapshot(snapshot, backing_file, new_snap_path)
    ...
    def extend_volume(self, volume, size_gb):
        ...
        self._check_extend_volume_support(volume_filename, active_image)
        self._extend_volume(volume_path, size_gb)

    def _extend_volume(self, volume_path, size_gb):
        image_utils.resize_image(volume_path, size_gb)

    def _check_extend_volume_support(self, volume_filename,
                                     active_file):
        if volume_filename != active_file:
