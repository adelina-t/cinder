...
class SmbFsTestCase(test.TestCase):
    ...
    def test_create_snapshot(self):
        ...
        self._smbfs_driver.create_snapshot(self._FAKE_SNAPSHOT)

        self._smbfs_driver._create_snapshot.assert_called_once_with(
    ...
    def _test_do_create_snapshot(self, unsupported_format=False):
        ...
        if unsupported_format:
            ...
            self.assertRaises(exception.InvalidVolume,
                              self._smbfs_driver._create_snapshot,
                              self._FAKE_SNAPSHOT,
                              self._FAKE_VOLUME_PATH,
                              self._FAKE_SNAPSHOT_PATH)
        else:
    ...
    def _test_delete_snapshot(self, is_active_image=True,
        ...
        if is_active_image:
            ...
            self._smbfs_driver.delete_snapshot(self._FAKE_SNAPSHOT)
    ...
    def _test_extend_volume(self, is_active_image=True):
        self._smbfs_driver.local_path = mock.Mock(
            return_value=self._FAKE_VOLUME_PATH)
    ...
    def test_extend_volume_with_snapshots(self):
        self._test_extend_volume(False)
    ...
    def _test_create_cloned_volume(self, is_available=True):
        if is_available:
            self._smbfs_driver.create_snapshot = mock.MagicMock()
            self._smbfs_driver._copy_volume_from_snapshot = mock.MagicMock()
            self._smbfs_driver.delete_snapshot = mock.MagicMock()
            expected = {'provider_location': self._FAKE_SHARE}

            ret_val = self._smbfs_driver.create_cloned_volume(
                self._FAKE_VOLUME, self._FAKE_VOLUME)
            self.assertTrue(self._smbfs_driver.create_snapshot.called)
            self.assertTrue(self._smbfs_driver.delete_snapshot.called)
            self.assertEqual(expected, ret_val)
        else:
